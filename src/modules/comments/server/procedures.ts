import { z } from "zod"
import { and, desc, eq, or, lt, getTableColumns, count, inArray, isNull, isNotNull } from "drizzle-orm";
...
export const commentsRouter = createTRPCRouter({
  ...
  getMany: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [user] = await db.
        select().from(users).where(inArray(users.clerkId, clerkUserId ? [clerkUserId] : []));
