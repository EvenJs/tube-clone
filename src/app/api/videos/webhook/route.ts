...
export const POST = async (request: Request) => {
  ...
  mux.webhooks.verifySignature(
    body, { "mux-signature": muxSignature }, SINGING_SECRET,
  )
  ...
  switch (payload.type as WebhookEvent["type"]) {
    case "video.asset.created": {
      ...
      await db.update(videos).set({ muxAssetId: data.id, muxStatus: data.status })
    ...
    case "video.asset.ready": {
      ...
      if (!uploadedPreview.data || !uploadedThumbnail.data) return new Response("Failed to upload thumbnail or preview", { status: 500 })
    ...
    case "video.asset.track.ready": {
      ...
      await db.update(videos).set({ muxTrackId: trackId, muxTrackStatus: status, })
